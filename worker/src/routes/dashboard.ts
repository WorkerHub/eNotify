...
dashboardRoutes.get("/stats", async (c) => {
  ...
  const subscriptionItems = allActiveItems.filter(
    (s) => s.item_kind === "subscription",
  );
  const regularItems = allActiveItems.filter(
    (s) => s.item_kind !== "subscription",
  );
  ...
  const payments = await listPaymentsByUserSince(
    c.env.DB,
    prefix,
    userId,
    sinceDate,
  ...
  for (const sub of subscriptionItems) {
    ...
    if (sub.category) {
      ...
      for (const cat of cats) {
        categoryExpense[cat] =
          (categoryExpense[cat] || 0) + monthly / cats.length;
      }
    }
  }
  ...
  return c.json({
    subscription: {
      ...
      monthly_average:
        Math.round((yearlyExpense / (currentMonth + 1)) * 100) / 100,
