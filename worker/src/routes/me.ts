...
} from "../db/queries/notification-history";
...
meRoutes.use(
  "/password",
  rateLimit({ max: 5, window: 300, keyPrefix: "pwd_change" }),
);
...
meRoutes.put("/password", async (c) => {
  ...
  if (accessTokenStr) {
    ...
    if (payload) {
      ...
      if (remaining > 0) {
        await c.env.KV.put(`bl:${payload.jti}`, "1", {
...
meRoutes.get("/notifications", async (c) => {
  ...
  const safeConfig: Record<string, any> = {
    enabled_channels: (() => {
      try {
        return JSON.parse(config.enabled_channels);
      } catch {
        return [];
      }
    })(),
    notification_hours: (() => {
      try {
        return JSON.parse(config.notification_hours);
      } catch {
        return [];
      }
    })(),
  ...
  for (const ch of channels) {
    ...
    safeConfig[`${ch}_configured`] = Object.entries(parsed).some(
      ([_k, v]) =>
        v !== "" &&
        v !== null &&
        v !== undefined &&
...
meRoutes.put("/notifications", async (c) => {
  ...
  if (body.enabled_channels !== undefined) {
    if (!Array.isArray(body.enabled_channels))
      return c.json({ error: "enabled_channels must be an array" }, 400);
    const validChannels = VALID_CHANNELS;
    if (
      body.enabled_channels.some((ch: string) => !validChannels.includes(ch))
    ) {
  ...
  if (body.notification_hours !== undefined) {
    ...
    if (
      body.notification_hours.some(
        (h: any) => !Number.isInteger(h) || h < 0 || h > 23,
      )
    ) {
      return c.json(
        { error: "notification_hours must contain integers 0-23" },
        400,
      );
    }
  ...
  for (const ch of channels) {
    if (body[`${ch}_config`]) {
      const existingConfig = existing
        ? JSON.parse((existing as any)[`${ch}_config`] || "{}")
        : {};
...
meRoutes.post("/notifications/token", async (c) => {
  ...
  await c.env.KV.put(`notify_token:${token}`, userId, {
...
meRoutes.get("/sessions", async (c) => {
  ...
  if (staleJtis.length > 0) {
    ...
    if (remaining.length === 0) {
      ...
      await c.env.KV.put(key, JSON.stringify(remaining), {
