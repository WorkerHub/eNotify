...
authRoutes.use(
  "/login",
  rateLimit({ max: 10, window: 300, keyPrefix: "login" }),
);
authRoutes.use(
  "/register",
  rateLimit({ max: 5, window: 300, keyPrefix: "register" }),
);
authRoutes.use(
  "/refresh",
  rateLimit({ max: 20, window: 300, keyPrefix: "refresh" }),
);
authRoutes.use(
  "/email/resend",
  rateLimit({ max: 3, window: 300, keyPrefix: "email_resend" }),
);
authRoutes.use(
  "/password/forgot",
  rateLimit({ max: 3, window: 300, keyPrefix: "pwd_forgot" }),
);
authRoutes.use(
  "/password/reset",
  rateLimit({ max: 5, window: 300, keyPrefix: "pwd_reset" }),
);
...
authRoutes.post("/register", async (c) => {
  ...
  const registrationEnabled = await getSetting(
    c.env.DB,
    prefix,
  ...
  if (emailVerificationEnabled === "true") {
    ...
    await c.env.KV.put(`email_verify:${verifyToken}`, id, {
      expirationTtl: 86400,
    });
...
authRoutes.post("/email/resend", async (c) => {
  ...
  const verifyToken = generateId();
  await c.env.KV.put(`email_verify:${verifyToken}`, user.id, {
    expirationTtl: 86400,
  });
...
export async function removeSessionIndex(
  kv: KVNamespace,
  userId: string,
  jti: string,
): Promise<void> {
