...
export async function handleScheduled(env: Env): Promise<void> {
  ...
  for (let i = 0; i < activeUsers.length; i += BATCH_SIZE) {
    ...
    await Promise.allSettled(
      batch.map((user) => processUser(env, prefix, db, kv, user)),
    );
  }
}
...
async function processUser(
  ...
  db: D1Database,
  kv: KVNamespace,
  user: { id: string; timezone?: string; language?: string },
): Promise<void> {
  try {
    ...
    const allowedHours: number[] = JSON.parse(
      notifyConfig.notification_hours || "[]",
    );
    ...
    for (const sub of subscriptions) {
      ...
      try {
        itemHours = JSON.parse(sub.notification_hours || "[]");
      } catch {
        itemHours = [];
      }
...
async function processSubscription(
  ...
  try {
    itemChannels = JSON.parse(sub.channels || "[]");
  } catch {
    itemChannels = [];
  }
  ...
  for (const { date: checkDate, label } of checkDates) {
    ...
    const lunarStr =
      sub.calendar_mode !== "solar" ? lunarLabel(checkDate) : null;
    ...
    if (lang === "zh") {
      ...
      const lines = [`名称：${sub.name}`, `模式：${modeLabel}`];
      ...
      const lines = [`Name: ${sub.name}`, `Mode: ${modeLabel}`];
      if (isBoth && label === "lunar" && lunarStr) {
