...
export async function generateTOTPSecret(
  ...
  const uri = `otpauth://totp/enotify:${encodeURIComponent(email)}?secret=${secret}&issuer=enotify&algorithm=SHA1&digits=6&period=30`;
  return { secret, uri };
}

export async function verifyTOTPAsync(
  secret: string,
  code: string,
  env?: Env,
  userId?: string,
...
async function generateTOTPCodeAsync(
  secret: string,
  counter: number,
): Promise<string> {
...
export async function sendOTPEmail(
  ...
  const subject =
    language === "zh" ? "eNotify 验证码" : "eNotify Verification Code";
  const html =
    language === "zh"
