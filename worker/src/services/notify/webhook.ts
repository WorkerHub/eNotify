...
export async function sendWebhook(configJson: string, message: NotifyMessage, _env: Env): Promise<{ success: boolean; error?: string }> {
  ...
  if (isPrivateHostname(parsed.hostname)) {
    return { success: false, error: 'Webhook URL must not point to private/internal addresses' }
  }

  const method = config.method || 'POST'
  let headers: Record<string, string> = {}
  if (config.headers) {
    try { headers = JSON.parse(config.headers) } catch { /* ignore invalid headers */ }
  }
  ...
  if (!response.ok) {
    ...
    return { success: false, error: `Webhook error (${response.status}): ${err}` }
  }
