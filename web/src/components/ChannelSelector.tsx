          ...
          <div className="p-2 border-b bg-muted/30">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                ...
                >
                  {selected.length === availableChannels.length ? t('common.clear') : t('common.selectAll')}
                </button>
                {selected.length > 0 && (
                  <button
                    type="button"
            ...
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  <span className="text-sm flex-1">{t('channels.allChannels')}</span>
                  {selected.length === 0 && <Check className="w-4 h-4 text-primary" />}
                </div>
                ...
                {availableChannels.map((channel) => (
                  ...
                  >
                    <div className="w-4 h-4 rounded-full border flex items-center justify-center">
                      {selected.includes(channel.id) && <Radio className="w-3 h-3" />}
                    </div>
                    <span className="text-sm flex-1">{channel.label}</span>
                    {selected.includes(channel.id) && <Check className="w-4 h-4 text-primary" />}
                  </div>
                ))}
              </>
            )}
