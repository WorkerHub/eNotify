...
function ProgressBar({
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      <span className="w-24 truncate text-muted-foreground shrink-0">
        {name}
      </span>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({ length: 4 }, (_, i) => `skeleton-card-${i + 1}`).map(
            (skeletonKey) => (
        ...
        <div className="bg-card rounded-xl border p-4 animate-pulse space-y-3">
          {Array.from({ length: 4 }, (_, i) => `skeleton-row-${i + 1}`).map(
            (skeletonKey) => (
              <div key={skeletonKey} className="h-4 bg-muted rounded" />
            ),
          )}
        </div>
      </div>
